...
void loop() {
  ...
  while (waitingNext) {
    ...
    // If button is pressed, exit the loop immediately
    if(previousButtonState == HIGH && currentButtonState == LOW){
      waitingNext = false; 
      
      // Stop the music and clear the LEDs so the next round starts clean
      noTone(buzzerPin);
      for(int i = 0; i < 4; i++){
        digitalWrite(ledPin[i], LOW);
      }
    ...
    // Every 200ms, advance the song and the LED dance (Non-blocking)
    if (waitingNext && (millis() - lastDanceUpdate >= 200)) {
      lastDanceUpdate = millis();

      // Play the next note for a short duration
      ...
      // Figure out which LED to turn on (bounce back and forth: 0, 1, 2, 3, 2, 1)
      ...
      // Move to the next step, loop back to 0 if at the end
      danceStep++;
